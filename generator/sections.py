# -*- coding: utf-8 -*-
from common import (WIDTH, MONO, SANS, esc, svg_open, frame_defs, decode_text, static_text,
                     rain_columns, panel_rain, chip, CHIP_PULSE_CSS, Phaser)

ROLES = ["Flutter Developer", "Node.js Backend Engineer", "React & TypeScript Dev", "Software Engineer"]
GLITCH_CYCLE = 20.0  # each headline re-glitches roughly once every 20s — kept to a handful of elements total
GLITCH = dict(cycle=GLITCH_CYCLE, steps=3, gap=0.6)


def header_svg(c, theme):
    h = 320
    uid = "hdr"
    ph = Phaser(GLITCH_CYCLE, slots=6)
    css_rain, rain_svg = rain_columns(uid, 1 if theme == "dark" else 2, c["rain_head"], c["rain_body"], WIDTH, h, n_cols=30, rows=8)
    css_name, name_svg = decode_text(uid, "name", "JAVOHIR ABDUVAHHOBOV", WIDTH/2, h/2 - 6, 42,
                                      c["text"], c["accent"], anchor="middle", spacing="1.5",
                                      seed=101, delay=ph.delay(), **GLITCH)
...
def whoami_svg(c, theme):
    h = 230
    uid = "who"
    ph = Phaser(GLITCH_CYCLE, slots=8)
    css_rain, rain_svg = panel_rain(uid, 11, c, WIDTH, h, opacity=0.14)
    label_css, label_svg = decode_text(uid, "lbl", "01 // WHOAMI", 34, 44, 15, c["accent"], c["text3"],
                                        anchor="start", spacing="2", seed=201, delay=ph.delay(), **GLITCH)
    ...
    for i, line in enumerate(lines):
        para_svg.append(static_text(line, 34, 90 + i * 26, 16.5, c["text"], weight="500"))
    ...
    for i, t in enumerate(tags):
        _, el, w = chip(c, tx, ty - 14, t, uid=uid, key=f"tag{i}")
        tag_svg.append(el)
        tx += w + 12
...
def ecosystem_svg(c, theme):
    h = 420
    uid = "eco"
    ph = Phaser(GLITCH_CYCLE, slots=10)
    css_rain, rain_svg = panel_rain(uid, 12, c, WIDTH, h, opacity=0.14)
    label_css, label_svg = decode_text(uid, "lbl", "02 // ECOSYSTEM", 34, 44, 15, c["accent"], c["text3"],
                                        anchor="start", spacing="2", seed=401, delay=ph.delay(), **GLITCH)

    def box(key, x, y, w, bh, title, sub):
        tsvg = static_text(title, x + w/2, y + bh/2 - 3, 14, c["text"], anchor="middle", weight="700")
        ssvg = static_text(sub, x + w/2, y + bh/2 + 16, 11, c["text3"], anchor="middle", weight="500")
        return f'''<g>
...
def stack_svg(c, theme):
    ...
    uid = "stk"
    ph = Phaser(GLITCH_CYCLE, slots=16)
    css_rain, rain_svg = panel_rain(uid, 13, c, WIDTH, h, opacity=0.14)
    label_css, label_svg = decode_text(uid, "lbl", "03 // STACK", 34, 44, 15, c["accent"], c["text3"],
                                        anchor="start", spacing="2", seed=501, delay=ph.delay(), **GLITCH)
    rows = []
    y = 78
    for gi, (group, items) in enumerate(STACK_GROUPS):
        rows.append(static_text(group, 34, y, 12, c["text3"], weight="600", spacing="1.5"))
        x = 34
        cy = y + 16
        for ii, item in enumerate(items):
            _, el, w = chip(c, x, cy, item, uid=uid, key=f"g{gi}i{ii}")
            rows.append(el)
            x += w + 10
        y += row_h
...
def transmission_svg(c, theme, email="javohirabduvahhobov@gmail.com"):
    ...
    uid = "tra"
    ph = Phaser(GLITCH_CYCLE, slots=6)
    css_rain, rain_svg = panel_rain(uid, 14, c, WIDTH, h, opacity=0.14)
    label_css, label_svg = decode_text(uid, "lbl", "04 // OPEN A CHANNEL", 34, 44, 15, c["accent"], c["text3"],
                                        anchor="start", spacing="2", seed=601, delay=ph.delay(), **GLITCH)
    status_svg = static_text("AVAILABLE FOR OPPORTUNITIES", 78, 105, 12.5, c["good"], weight="600", spacing="1.5")
    email_svg = static_text(email, 34, 150, 22, c["text"], weight="700")
...
def soundtrack_svg(c, theme, track="We Do What We Want (Edit)", artist="Alan Fitzpatrick",
    ...
    h = 130
    uid = "snd"
    css_rain, rain_svg = panel_rain(uid, 15, c, WIDTH, h, opacity=0.14)
    title_css, title_svg = decode_text(uid, "title", track, 96, 62, 17, c["text"], c["accent"],
                                        anchor="start", weight="700", seed=801, **GLITCH)
    artist_svg = static_text(artist, 96, 82, 13, c["text2"], weight="500")
...
def footer_svg(c, theme):
    h = 150
    uid = "ftr"
    css_rain, rain_svg = rain_columns(uid, 3 if theme == "dark" else 4, c["rain_head"], c["rain_body"], WIDTH, h, n_cols=30, rows=7)
