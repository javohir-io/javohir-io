...
def static_text(text, x, y, size, color, anchor="start", weight="600", family=None, spacing=None):
    """Plain, non-animated text — used for anything that doesn't need to be a focal glitch point."""
    ...
    return f'<text {attrs} fill="{color}">{esc(text)}</text>'


class Phaser:
    """Spreads many decode-text elements' glitch moments across the cycle instead of
    all firing at once — smoother/cheaper to render, and reads as different words
    glitching at different times rather than a synchronized flash."""
    def __init__(self, cycle, slots=10):
        self.cycle = cycle
        self.slots = slots
        self.i = 0

    def delay(self):
        d = -(self.i % self.slots) * (self.cycle / self.slots)
        self.i += 1
        return round(d, 3)


def decode_text(uid, key, text, x, y, size, color, ghost, anchor="start",
                 weight="700", family=None, spacing=None, cycle=10.0,
                 frame_dur=0.055, steps=4, gap=0.4, seed=None, delay=0.0):
    """Returns (css_rules, svg_markup) for a per-character 'decode from noise' loop.
    `delay` (seconds, usually <= 0) offsets this element's position within the shared
    cycle so not every text glitches at the same instant."""
    family = family or MONO
    ...
    for i, frame in enumerate(frames):
        ...
        style = (f'animation:{name} {cycle}s steps(1) {delay}s infinite;')
...
def chip(c, x, y, text, uid="chp", key="", w=None, h=28, delay=0.0, dur=10.0, seed=None):
    """Chip pill — plain static label. Chips no longer glitch individually (too many
    at once was the main source of lag); the section label above them still does."""
    w = w or (len(text) * 7.2 + 26)
    txt_svg = static_text(text, x + w / 2, y + h / 2 + 4, 12.5, c["text"], anchor="middle", weight="600")
    box = f'''<g>
...
def panel_rain(uid, seed, c, w, h, n_cols=22, opacity=0.14):
    """A subtle full-panel letter/number rain layer, meant to sit under content."""
    css, svg = rain_columns(uid, seed, c["rain_head"], c["rain_body"], w, h, n_cols=n_cols, rows=max(5, h // 26))
