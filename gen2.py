# -*- coding: utf-8 -*-
import os, html, math, random, json
...
@keyframes jit{uid} {{ 0%,100% {{ transform:translate(0,0) }} 22% {{ transform:translate(0.7px,-0.6px) }}
  41% {{ transform:translate(-0.6px,0.5px) }} 63% {{ transform:translate(0.6px,0.6px) }}
  78% {{ transform:translate(-0.5px,-0.7px) }} 90% {{ transform:translate(0.4px,0) }} }}
.jit{uid} {{ display:inline-block; animation: jit{uid} 0.22s steps(1,end) infinite; }}
@keyframes flash{uid} {{ 0%,93%,100% {{ opacity:0 }} 93.6% {{ opacity:0.16 }} 94% {{ opacity:0 }}
  94.6% {{ opacity:0.22 }} 95% {{ opacity:0 }} 96.4% {{ opacity:0.1 }} 96.8% {{ opacity:0 }} }}
.flash{uid} {{ animation: flash{uid} 7s linear infinite; }}
@keyframes grain{uid} {{ 0%,100% {{ opacity:0.035 }} 30% {{ opacity:0.07 }} 55% {{ opacity:0.02 }} 80% {{ opacity:0.06 }} }}
.grain{uid} {{ animation: grain{uid} 0.6s steps(2,end) infinite; }}
</style>'''
...
def laughing_skull(c, uid, cx, cy, scale=1.0):
    """
    A stylised skull icon — no horns, angry brow, narrow slanted eyes, a
    wide fanged grin — built from overlapping solid shapes with the jaw as
    an independently animated piece so it visibly cackles rather than just
    sitting there. Hand-authored, not vector-traced from the reference.
    """
    s = scale
    out = [f'<g transform="translate({cx},{cy}) scale({s})">']

    # soft red glow behind the whole thing, pulsing harder than a calm idle icon would
    out.append(f'<circle cx="0" cy="-6" r="42" fill="url(#core{uid})" opacity="0.4">'
                f'<animate attributeName="opacity" values="0.28;0.6;0.28" dur="1.6s" repeatCount="indefinite"/></circle>')

    # sharp, jittery rock — angrier and faster than a gentle chuckle-shake
    out.append(f'<g>'
                f'<animateTransform attributeName="transform" type="rotate" '
                f'values="0;-3;1;-4;0;3;-1;0" keyTimes="0;0.14;0.28;0.42;0.56;0.7;0.85;1" '
                f'dur="1.1s" repeatCount="indefinite"/>')

    fill = c["accent"]
    # cranium — no horns; a couple of thin scar cracks give it menace instead
    out.append(f'<ellipse cx="0" cy="-8" rx="24" ry="21" fill="{fill}"/>')
    out.append(f'<path d="M -14 -20 L -8 -8 L -12 -2" fill="none" stroke="#000000" stroke-width="1.4" '
                f'stroke-linecap="round" opacity="0.55"/>')
    out.append(f'<path d="M 12 -18 L 16 -10" fill="none" stroke="#000000" stroke-width="1.3" '
                f'stroke-linecap="round" opacity="0.5"/>')

    # cheeks tapering to a jagged, wider upper-teeth edge (static, upper jaw)
    out.append(f'<path d="M -22 -4 L -23 6 L -18 2 L -14 11 L -9 4 L -5 10 L 0 3 '
                f'L 5 10 L 9 4 L 14 11 L 18 2 L 23 6 L 22 -4 Z" fill="{fill}"/>')

    # angry brow — a thick downward-slanting slash over each eye, meeting toward the nose
    out.append(f'<path d="M -19 -17 L -3 -11 L -4 -8 L -20 -14 Z" fill="#000000"/>')
    out.append(f'<path d="M 19 -17 L 3 -11 L 4 -8 L 20 -14 Z" fill="#000000"/>')
    # furrow between the brows
    out.append(f'<line x1="-1.4" y1="-13" x2="-1.4" y2="-9" stroke="#000000" stroke-width="1" opacity="0.7"/>')
    out.append(f'<line x1="1.4" y1="-13" x2="1.4" y2="-9" stroke="#000000" stroke-width="1" opacity="0.7"/>')

    # narrow, angry, slanted eye sockets
    out.append(f'<g transform="rotate(-20)"><ellipse cx="-10" cy="-6" rx="6" ry="4.6" fill="#000000"/></g>')
    out.append(f'<g transform="rotate(20)"><ellipse cx="10" cy="-6" rx="6" ry="4.6" fill="#000000"/></g>')
    for ex in (-10, 10):
        out.append(f'<circle cx="{ex}" cy="-6" r="2.1" fill="{c["accent3"]}" opacity="0.9">'
                    f'<animate attributeName="opacity" values="0.5;1;0.5" dur="0.6s" repeatCount="indefinite"/>'
                    f'<animate attributeName="r" values="1.7;2.4;1.7" dur="0.6s" repeatCount="indefinite"/></circle>')

    # nose notch
    out.append(f'<path d="M -4 2 L 4 2 L 0 8 Z" fill="#000000"/>')

    # jaw — the moving piece, wider jagged fanged edge, tapering to a chin
    out.append(f'<g>')
    out.append(f'<animateTransform attributeName="transform" type="translate" '
                f'values="0,0; 0,9; 0,1; 0,10; 0,0; 0,7; 0,0" '
                f'keyTimes="0;0.14;0.28;0.42;0.56;0.75;1" dur="1.3s" repeatCount="indefinite"/>')
    out.append(f'<path d="M -22 6 L -17 -2 L -12 6 L -8 0 L -4 7 L 0 -1 L 4 7 L 8 0 L 12 6 '
                f'L 17 -2 L 22 6 C 18 20 8 27 0 27 C -8 27 -18 20 -22 6 Z" fill="{fill}"/>')
    out.append('</g>')  # jaw group

    out.append('</g>')  # rock group

    # laugh-text flicker, timed to the jaw bounce — sharper "HA" for the meaner laugh
    out.append(f'<text x="28" y="2" font-size="10.5" font-weight="700" class="accent2 sans" opacity="0">HA'
                f'<animate attributeName="opacity" values="0;1;0;1;0;1;0;0" '
                f'keyTimes="0;0.14;0.28;0.42;0.56;0.75;0.9;1" dur="1.3s" repeatCount="indefinite"/></text>')

    out.append('</g>')  # outer translate/scale group
    return "\n".join(out)
...
def glitch_text(c, uid, x, y, text, fs, weight=700, period=3, begin=0, cls="ink sans", anchor="start"):
...
def decode_text(c, uid, x, y, text, fs, begin=0, cycle=8, cls="ink", charw_factor=0.61, glyph_seed=None):
    """
...
def section_head(c, num, title, x=40, y=54, sub=None, uid=None):
    out = [hex_badge(c, x+16, y-6, num, spin=True, dur=26)]
    jit_cls = f' jit{uid}' if uid else ''
    out.append(f'<text x="{x+44}" y="{y}" font-size="19" class="ink sans{jit_cls}" font-weight="700">{esc(title)}</text>')
    if sub:
        out.append(f'<text x="{x+44}" y="{y+19}" font-size="11.5" class="dim">{esc(sub)}</text>')
    return "\n".join(out)

def whisper(c, uid, x, y, text, cycle=15, begin=0, cls="dim2", anchor="start"):
    """a phrase that flashes in for an instant then vanishes for the rest of the loop — unsettling, easy to miss"""
    ta = f' text-anchor="{anchor}"' if anchor != "start" else ""
    kt = "0;0.035;0.06;0.075;1"
    return (f'<text x="{x}" y="{y}" font-size="10.5" class="{cls}"{ta} opacity="0">{esc(text)}'
            f'<animate attributeName="opacity" values="0;0;0.6;0;0" keyTimes="{kt}" dur="{cycle}s" begin="{begin}s" repeatCount="indefinite"/></text>')
...
def panel(c, h, uid, w=WIDTH, m=4, rx=20, sweep_dur=6, sweep_begin=0, glitch_begin=1.5, scan_begin=0.4, radar=None):
    ...
    out.append(f'<g clip-path="url(#clip{uid})">'
                ...
                f'</rect></g>')
    # constant low-level grain — always flickering faintly, not just a periodic burst
    out.append(f'<g clip-path="url(#clip{uid})">'
                f'<rect x="{m}" y="{m}" width="{pw}" height="{ph}" filter="url(#noise{uid})" class="grain{uid}"/>'
                f'</g>')
    out.append(scanline(c, uid, m, m, pw, ph, period=1.7 + (stable_hash(uid) % 5) * 0.2, begin=scan_begin))
    out.append(static_burst(c, uid, m, m, pw, ph, period=3.4 + (stable_hash(uid) % 4) * 0.6, begin=glitch_begin))
    # rare full-panel corrupted-frame flash — a CRT-style misfire
    out.append(f'<g clip-path="url(#clip{uid})">'
                f'<rect x="{m}" y="{m}" width="{pw}" height="{ph}" fill="{c["accent"]}" class="flash{uid}"/>'
                f'</g>')
    if radar:
        rx_, ry_, rr_ = radar
...
def arrow(c, x1, y1, x2, y2, label=None, flow_dur=1.1, flow_begin=0, flicker=False):
    ...
    if flicker:
        ...
        out.append(f'<animate attributeName="opacity" values="1;1;0.15;0.15;1;1;1" keyTimes="{kt}" dur="4.5s" begin="1.8s" repeatCount="indefinite"/>')
...
def header_svg(c):
    ...
    name_svg, _ = decode_text(c, uid, 40, 118, "JAVOHIR ABDUVAHHOBOV", 32, begin=0.4, cycle=9, cls="ink sans", glyph_seed=1)
    s.append(f'<g font-weight="800" font-family="{SANS}" class="jit{uid}">{name_svg}</g>')
    s.append(glitch_text(c, uid, 40, 146, "Full-stack & Mobile Developer", 15.5, weight=600, period=3.4, begin=1.1, cls="muted sans"))
    s.append(f'<text x="40" y="170" font-size="12" class="dim">Flutter · Node.js · React · PostgreSQL · Python · ML / DL</text>')
    s.append(whisper(c, uid, WIDTH-40, 118, "// it is still compiling", cycle=17, begin=6, cls="dim2", anchor="end"))
    ...
    s.append(f'<text x="{tx+320}" y="{ty+24}" font-size="12" class="muted">{esc(base)}'
              f'<animate attributeName="opacity" values="1;1;0;0;1;1;1" keyTimes="{kt}" dur="4.2s" begin="1.4s" repeatCount="indefinite"/></text>')
    s.append(f'<text x="{tx+320}" y="{ty+24}" font-size="12" fill="{c["accent"]}" opacity="0">{esc(glitched)}'
              f'<animate attributeName="opacity" values="0;0;1;1;0;0;0" keyTimes="{kt}" dur="4.2s" begin="1.4s" repeatCount="indefinite"/></text>')
...
def whoami_svg(c):
    ...
    s.append(section_head(c, "01", "whoami", uid=uid))
    ...
    s.append(whisper(c, uid, WIDTH-40, frow_y+34, "// something else is reading this too", cycle=21, begin=9, cls="dim2", anchor="end"))
...
def ecosystem_svg(c):
    ...
    s.append(section_head(c, "02", "ecosystem", sub="system map \u2014 how the pieces talk to each other", uid=uid))
...
def stack_svg(c):
    ...
    s.append(section_head(c, "03", "stack", uid=uid))
    groups = [
    ...
    # currently learning — styled differently on purpose: dashed border, unsettled pulse, not "mastered"
    s.append(f'<line x1="40" y1="{y-2}" x2="{WIDTH-40}" y2="{y-2}" class="line" stroke-width="1" stroke-dasharray="2,5"/>')
    y += 20
    s.append(f'<circle cx="46" cy="{y+9}" r="2.6" fill="{c["accent3"]}"><animate attributeName="opacity" values="0.2;1;0.2" dur="1.1s" repeatCount="indefinite"/></circle>')
    s.append(f'<text x="56" y="{y+13}" font-size="10.5" class="accent3" letter-spacing="0.5">learning</text>')
    label = "Docker"
    pw = tw(label, 12) + 22
    px, py = 152, y
    s.append(f'<rect x="{px}" y="{py}" width="{pw:.1f}" height="24" rx="5" fill="{c["panel"]}" stroke="{c["accent3"]}" stroke-width="1.3" stroke-dasharray="4,3">'
              f'<animate attributeName="stroke-opacity" values="0.4;1;0.4" dur="1.8s" repeatCount="indefinite"/></rect>')
    s.append(f'<text x="{px+11}" y="{py+16.5}" font-size="12" class="ink">{esc(label)}</text>')
    tagx = px + pw + 10
    tag = "in progress"
    tw_tag = tw(tag, 9.5) + 16
    s.append(f'<rect x="{tagx:.1f}" y="{py+3}" width="{tw_tag:.1f}" height="17" rx="8.5" fill="none" stroke="{c["accent3"]}" stroke-width="1"/>')
    s.append(f'<text x="{tagx+8:.1f}" y="{py+14.5}" font-size="9.5" class="accent3">{esc(tag)}</text>')
    s.append(f'<text x="{tagx+tw_tag+16:.1f}" y="{py+16.5}" font-size="10.5" class="dim">containerizing the backend + a consistent local Postgres setup</text>')
    s.append("</svg>")
    return "\n".join(s)
...
def matrix_footer_svg(c):
    ...
    s.append(bottom_svg)
    s.append(whisper(c, uid, 48, 145, "// you are being logged as: viewer_0x" + format(stable_hash("javohir-io") % 4096, "04X"), cycle=19, begin=8, cls="dim2"))

    kt = "0;0.75;0.76;0.8;0.81;0.85;1"
    s.append(f'<text x="{WIDTH-40}" y="82" font-size="10.5" class="dim" text-anchor="end">// EOF'
              f'<animate attributeName="opacity" values="1;1;0;0;1;1;1" keyTimes="{kt}" dur="5s" begin="2s" repeatCount="indefinite"/></text>')
    s.append(f'<text x="{WIDTH-40}" y="82" font-size="10.5" fill="{c["accent"]}" text-anchor="end" opacity="0">// 0x5F'
              f'<animate attributeName="opacity" values="0;0;1;1;0;0;0" keyTimes="{kt}" dur="5s" begin="2s" repeatCount="indefinite"/></text>')
...
# ---------------------------------------------------------------- SOUNDTRACK (click-through to Spotify — no audio plays inside GitHub)
def soundtrack_svg(c):
    """
    GitHub strips <iframe>/<audio>/JS from README SVGs and markdown alike, so
    no audio can actually play inside the page itself — this is a platform
    limit, not something any tool can work around. What this *can* do: look
    alive (spinning vinyl, a live equalizer) and be a real, working link —
    the whole panel gets wrapped in <a href="..."> in the README, so clicking
    it genuinely opens Spotify and starts playback there.

    EDIT: change SPOTIFY_URL in the README's <a href> to your actual profile
    or playlist link before publishing — it's a placeholder right now.
    """
    h = 210
    uid = "m"
    s = [svg_open(h), defs_and_style(c, uid), panel(c, h, uid, sweep_dur=9, sweep_begin=-2)]

    title_svg, _ = decode_text(c, uid, 40, 44, "WHO ARE YOU, REALLY?", 19, begin=0.2, cycle=8, cls="ink sans", glyph_seed=21)
    s.append(f'<g font-weight="800">{title_svg}</g>')
    s.append(f'<text x="40" y="63" font-size="11" class="dim">Mikky Ekko \u2014 GitHub won\u2019t play audio here, so click through and actually hit play'
              f'<tspan class="cursor{uid}">_</tspan></text>')

    # spinning vinyl
    vx, vy, vr = 92, 138, 40
    s.append(f'<g>'
              f'<animateTransform attributeName="transform" type="rotate" from="0 {vx} {vy}" to="360 {vx} {vy}" dur="5s" repeatCount="indefinite"/>'
              f'<circle cx="{vx}" cy="{vy}" r="{vr}" fill="#000000" stroke="{c["line"]}" stroke-width="1.2"/>'
              + "".join(f'<circle cx="{vx}" cy="{vy}" r="{rr}" fill="none" stroke="{c["dim2"]}" stroke-width="0.6" opacity="0.5"/>' for rr in (30, 22, 14))
              + f'<circle cx="{vx}" cy="{vy}" r="9" fill="{c["accent"]}"/>'
              + f'<circle cx="{vx}" cy="{vy}" r="2" fill="#000000"/>'
              + f'<rect x="{vx-1}" y="{vy-vr}" width="2" height="{vr*0.4:.0f}" fill="{c["ink2"]}" opacity="0.5"/>'
              + '</g>')
    # tonearm resting on the edge, static
    s.append(f'<line x1="{vx+52}" y1="{vy-52}" x2="{vx+vr-6}" y2="{vy-vr+8}" stroke="{c["dim"]}" stroke-width="2.4" stroke-linecap="round"/>')
    s.append(f'<circle cx="{vx+52}" cy="{vy-52}" r="4.5" fill="{c["dim"]}"/>')
    s.append(f'<circle cx="{vx+vr-6}" cy="{vy-vr+8}" r="2.5" fill="{c["accent"]}"/>')

    # live-looking equalizer bars
    eqx = 200
    r = random.Random(stable_hash(uid) + 3)
    heights = [r.uniform(14, 40) for _ in range(14)]
    for i, hh in enumerate(heights):
        dur = round(r.uniform(0.45, 0.9), 2)
        delay = round(r.uniform(0, 0.6), 2)
        bx = eqx + i * 12
        s.append(f'<rect x="{bx}" y="{158-hh:.1f}" width="6" height="{hh:.1f}" rx="2" fill="{c["accent"]}">'
                  f'<animate attributeName="height" values="{hh:.1f};{hh*0.25:.1f};{hh:.1f}" dur="{dur}s" begin="{delay}s" repeatCount="indefinite"/>'
                  f'<animate attributeName="y" values="{158-hh:.1f};{158-hh*0.25:.1f};{158-hh:.1f}" dur="{dur}s" begin="{delay}s" repeatCount="indefinite"/>'
                  f'</rect>')
    s.append(f'<text x="{eqx}" y="178" font-size="10.5" class="dim">Mikky Ekko \u00b7 Reds \u00b7 2010 \u00b7 open in Spotify to actually hear it</text>')

    # big call-to-action pill (the README wraps the whole image in <a href>)
    btn_label = "\u25b6  OPEN ON SPOTIFY"
    bw = tw(btn_label, 13.5) + 40
    bx, by = WIDTH - 40 - bw, 128
    s.append(f'<rect x="{bx:.1f}" y="{by}" width="{bw:.1f}" height="38" rx="19" fill="{c["accent"]}">'
              f'<animate attributeName="opacity" values="0.9;1;0.9" dur="1.6s" repeatCount="indefinite"/></rect>')
    s.append(f'<rect x="{bx:.1f}" y="{by}" width="{bw:.1f}" height="38" rx="19" fill="none" stroke="{c["accent"]}" stroke-width="1.5" opacity="0">'
              f'<animate attributeName="opacity" values="0.6;0" dur="1.6s" repeatCount="indefinite"/>'
              f'<animate attributeName="width" values="{bw:.1f};{bw+18:.1f}" dur="1.6s" repeatCount="indefinite"/>'
              f'<animate attributeName="x" values="{bx:.1f};{bx-9:.1f}" dur="1.6s" repeatCount="indefinite"/>'
              f'<animate attributeName="height" values="38;46" dur="1.6s" repeatCount="indefinite"/>'
              f'<animate attributeName="y" values="{by};{by-4}" dur="1.6s" repeatCount="indefinite"/></rect>')
    s.append(f'<text x="{bx+bw/2:.1f}" y="{by+24}" text-anchor="middle" font-size="13.5" font-weight="700" '
              f'fill="{c["white"]}" class="sans">{esc(btn_label)}</text>')

    s.append("</svg>")
    return "\n".join(s)

# ---------------------------------------------------------------- ASTRONAUT (real mesh, extracted from your .glb)
def _load_astronaut_mesh():
    mesh_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "astronaut_mesh.json")
    with open(mesh_path) as f:
        d = json.load(f)
    return [tuple(v) for v in d["verts"]], [tuple(e) for e in d["edges"]]

def astronaut_svg(c):
    """
    Not a stand-in shape this time — these are the real vertex positions
    pulled straight out of horror_game_astronaut.glb (16 mesh parts: helmet,
    visor, backpack, belt, armor plates, tubing, all already correctly
    positioned relative to each other in the source file), downsampled to a
    manageable edge budget and run through the same rotation-matrix +
    perspective-projection + SMIL-keyframe-baking pipeline as everything
    else 3D in this set.
    """
    h = 600
    uid = "n"
    ...
    verts, edges = _load_astronaut_mesh()
    edge_frames, vert_frames = a3d.bake_wireframe(
        verts, edges, frames=56, cx=cx, cy=cy, scale=300, wx=0, wy=2, wz=0
    )

    s = [svg_open(h), defs_and_style(c, uid), panel(c, h, uid, sweep_dur=10, sweep_begin=-5)]

    title_svg, _ = decode_text(c, uid, 40, 44, "UNIT RECOVERED", 19, begin=0.2, cycle=8, cls="ink sans", glyph_seed=23)
    s.append(f'<g font-weight="800">{title_svg}</g>')
    s.append(f'<text x="40" y="63" font-size="11" class="dim">extracted from horror_game_astronaut.glb \u2014 1,240 verts \u00b7 645 edges, live-rotated'
              f'<tspan class="cursor{uid}">_</tspan></text>')

    # faint depth-cue rings behind it
    for rr, op in [(210, 0.08), (170, 0.12)]:
        s.append(f'<circle cx="{cx}" cy="{cy+6}" r="{rr}" fill="none" stroke="{c["accent"]}" stroke-width="1" opacity="{op}"/>')

    s.append(f'<g clip-path="url(#clip{uid})">')
    s.append(a3d.render_wireframe_svg(edge_frames, vert_frames, dur=26, c=c, edge_color_key="accent", vert_color_key="accent3"))
    s.append('</g>')

    s.append(f'<text x="{WIDTH-40}" y="{h-24}" text-anchor="end" font-size="10" class="dim">no distress signal detected</text>')
    s.append("</svg>")
    return "\n".join(s)

BANNERS = {
    "header.svg": header_svg,
    "astronaut.svg": astronaut_svg,
    "soundtrack.svg": soundtrack_svg,
