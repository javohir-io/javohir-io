...
def laughing_skull(c, uid, cx, cy, scale=1.0):
    """
    A stylised devil-skull icon, built as layered overlapping shapes (all the
    same solid fill so seams don't show), with the jaw as an independently
    animated piece so the mouth actually opens and closes in a quick, rough
    'chuckle' rhythm rather than just being a static icon. Hand-authored from
    the reference image rather than vector-traced (no trace tooling here),
    so it's a close stylistic recreation, not a pixel-exact copy.
    """
    s = scale
    out = [f'<g transform="translate({cx},{cy}) scale({s})">']

    # soft red glow behind the whole thing, breathing slowly
    out.append(f'<circle cx="0" cy="-10" r="46" fill="url(#core{uid})" opacity="0.35">'
                f'<animate attributeName="opacity" values="0.25;0.5;0.25" dur="2.4s" repeatCount="indefinite"/></circle>')

    # gentle overall rock, like shaking with laughter
    out.append(f'<g>'
                f'<animateTransform attributeName="transform" type="rotate" '
                f'values="0;-2;0;2;0" keyTimes="0;0.25;0.5;0.75;1" dur="1.4s" repeatCount="indefinite"/>')

    fill = c["accent"]
    # horns
    out.append(f'<path d="M -13 -20 C -25 -29 -33 -45 -22 -58 C -19 -46 -9 -35 -3 -28 '
                f'C -6 -25 -10 -22 -13 -20 Z" fill="{fill}"/>')
    out.append(f'<path d="M 13 -20 C 25 -29 33 -45 22 -58 C 19 -46 9 -35 3 -28 '
                f'C 6 -25 10 -22 13 -20 Z" fill="{fill}"/>')
    # cranium
    out.append(f'<ellipse cx="0" cy="-10" rx="25" ry="23" fill="{fill}"/>')
    # cheeks tapering to a jagged upper-teeth edge (static, upper jaw)
    out.append(f'<path d="M -23 -6 L -24 6 L -18 3 L -13 12 L -7 5 L 0 13 L 7 5 L 13 12 '
                f'L 18 3 L 24 6 L 23 -6 Z" fill="{fill}"/>')
    # eye sockets (angled, devilish)
    out.append(f'<g transform="rotate(-14)"><ellipse cx="-10" cy="-9" rx="6.4" ry="8.6" fill="#000000"/></g>')
    out.append(f'<g transform="rotate(14)"><ellipse cx="10" cy="-9" rx="6.4" ry="8.6" fill="#000000"/></g>')
    for ex in (-10, 10):
        out.append(f'<circle cx="{ex}" cy="-9" r="2.2" fill="{c["accent3"]}" opacity="0.85">'
                    f'<animate attributeName="opacity" values="0.5;1;0.5" dur="1.1s" repeatCount="indefinite"/></circle>')
    # nose notch
    out.append(f'<path d="M -4 -1 L 4 -1 L 0 6 Z" fill="#000000"/>')

    # jaw — the moving piece, jagged lower-teeth edge on top, tapering to a chin
    out.append(f'<g>')
    out.append(f'<animateTransform attributeName="transform" type="translate" '
                f'values="0,0; 0,7; 0,1; 0,8; 0,0; 0,6; 0,0" '
                f'keyTimes="0;0.14;0.28;0.42;0.56;0.75;1" dur="1.6s" repeatCount="indefinite"/>')
    out.append(f'<path d="M -18 9 L -13 2 L -7 9 L 0 1 L 7 9 L 13 2 L 18 9 '
                f'C 15 21 7 27 0 27 C -7 27 -15 21 -18 9 Z" fill="{fill}"/>')
    out.append('</g>')  # jaw group

    out.append('</g>')  # rock group

    # laugh-text flicker, timed loosely to the jaw bounce
    out.append(f'<text x="30" y="4" font-size="10" font-weight="700" class="accent2 sans" opacity="0">ha'
                f'<animate attributeName="opacity" values="0;1;0;1;0;1;0;0" '
                f'keyTimes="0;0.14;0.28;0.42;0.56;0.75;0.9;1" dur="1.6s" repeatCount="indefinite"/></text>')
