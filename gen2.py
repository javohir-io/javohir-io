...
def laughing_skull(c, uid, cx, cy, scale=1.0):
    """
    A cool, laughing skull — no horns, no angry brow, just a wide grin and a
    genuine laugh: the jaw independently bounces open/closed on a loose
    rhythm, the whole thing rocks gently, and each eye has a small glowing
    ember that pulses. Hand-authored, not vector-traced from any reference.
    """
    s = scale
    out = [f'<g transform="translate({cx},{cy}) scale({s})">']

    # soft red glow behind it
    out.append(f'<circle cx="0" cy="-6" r="40" fill="url(#core{uid})" opacity="0.35">'
                f'<animate attributeName="opacity" values="0.25;0.5;0.25" dur="2s" repeatCount="indefinite"/></circle>')

    # easy, amused rock — laughing, not raging
    out.append(f'<g>'
                f'<animateTransform attributeName="transform" type="rotate" '
                f'values="0;-2;0;2;0" keyTimes="0;0.25;0.5;0.75;1" dur="1.3s" repeatCount="indefinite"/>')

    fill = c["accent"]
    # cranium — no horns, no scars, just a clean dome
    out.append(f'<ellipse cx="0" cy="-8" rx="24" ry="21" fill="{fill}"/>')

    # cheeks tapering to a jagged upper-teeth edge (static, upper jaw)
    ...
    # friendly round eye sockets, glowing embers, no angry slant
    for ex in (-10, 10):
        out.append(f'<circle cx="{ex}" cy="-8" r="6.2" fill="#000000"/>')
        out.append(f'<circle cx="{ex}" cy="-8" r="2.3" fill="{c["accent3"]}" opacity="0.9">'
                    f'<animate attributeName="opacity" values="0.55;1;0.55" dur="1s" repeatCount="indefinite"/>'
                    f'<animate attributeName="r" values="1.9;2.6;1.9" dur="1s" repeatCount="indefinite"/></circle>')

    # nose notch
    out.append(f'<path d="M -4 1 L 4 1 L 0 7 Z" fill="#000000"/>')

    # jaw — the moving piece, jagged fanged edge, tapering to a chin
    out.append(f'<g>')
    out.append(f'<animateTransform attributeName="transform" type="translate" '
                f'values="0,0; 0,8; 0,1; 0,9; 0,0; 0,6; 0,0" '
                f'keyTimes="0;0.14;0.28;0.42;0.56;0.75;1" dur="1.4s" repeatCount="indefinite"/>')
    out.append(f'<path d="M -20 6 L -15 -1 L -10 6 L -6 0 L -3 7 L 0 -1 L 3 7 L 6 0 L 10 6 '
                f'L 15 -1 L 20 6 C 16 19 7 25 0 25 C -7 25 -16 19 -20 6 Z" fill="{fill}"/>')
    out.append('</g>')  # jaw group

    out.append('</g>')  # rock group

    # laugh-text flicker, timed to the jaw bounce
    out.append(f'<text x="26" y="0" font-size="10.5" font-weight="700" class="accent2 sans" opacity="0">HA'
                f'<animate attributeName="opacity" values="0;1;0;1;0;1;0;0" '
                f'keyTimes="0;0.14;0.28;0.42;0.56;0.75;0.9;1" dur="1.4s" repeatCount="indefinite"/></text>')
...
def header_svg(c):
    ...
    s.append(laughing_skull(c, uid, 478, 102, scale=0.62))
