...
# ---- black / white / red — pushed into more shades, sharper, more alive.
LIGHT = dict(
    ink="#0a0a0a", ink2="#000000",
    white="#ffffff", off="#f8f5ee",
    muted="#5a4a1f", dim="#8a7530", dim2="#c9b878",
    accent="#b8860b",     # deep gold — primary
    accent2="#5c4413",    # dark bronze — structural / secondary
    accent3="#ffd700",    # bright gold — glow / highlight channel
    accent4="#3d2e0a",    # near-black bronze — deepest shade
    line="#e8dfc4", line2="#d8c896", panel="#fffdf7", grid="#f1e8d0",
)
DARK = dict(
    ink="#f5efd8", ink2="#fff8e1",
    white="#0a0805", off="#050402",
    muted="#c9ad5a", dim="#8a7530", dim2="#4a3d18",
    accent="#ffd700",
    accent2="#b8860b",
    accent3="#fff2b0",
    accent4="#2a2007",
    line="#2e2612", line2="#1c170a", panel="#0a0805", grid="#161206",
)

# falling-letter rain + clarity-flash alphabet — plain alphanumerics and a
# few scientific symbols, not alien runes: this reskin is "genius", not "alien"
CLARITY_CHARS = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789∑∫π∞ΩΔ√%")


# redaction / cipher noise — deliberately not alien-looking: block shading,
# hex/binary digits, classic "corrupted classified document" characters
ALIEN = list("▓▒░█▄▀■□▪▫◆◇0123456789ABCDEF#%&@*!")
...
def radar_sweep(c, uid, cx, cy, r, dur=5, begin=0):
    """a rotating radar arm + fading rings, control-room scanner style"""
    out = [f'<g>']
...
def decode_text(c, uid, x, y, text, fs, begin=0, cycle=8, cls="ink", charw_factor=0.61, glyph_seed=None):
    """
    Per-character alive text: renders as redaction/cipher noise first, decodes into the
...
def matrix_rain(c, uid, x, y, w, h, cols=None, begin=0):
    """falling cipher/binary character columns, clipped to a region — a corrupted-feed look"""
...
def header_svg(c):
    ...
    s.append(f'<text x="70" y="57" font-size="12.5" class="accent" letter-spacing="0.5">clearance confirmed // channel secured</text>')
    ...
    base = "$ status --check → building . expanding . undetected"
    glitched = "$ status --check → trace . flagged . evading"
...
def reactor_svg(c):
    ...
    title_svg, _ = decode_text(c, uid, 40, 48, "OPERATIONS CORE", 20, begin=0.2, cycle=15, cls="ink sans", glyph_seed=2)
    s.append(f'<g font-weight="800">{title_svg}</g>')
    s.append(f'<text x="40" y="68" font-size="11.5" class="dim">dossier compiled // cross-referencing assets'
...
def transmission_svg(c):
    ...
    title_svg, _ = decode_text(c, uid, 40, 76, "OPEN A CHANNEL", 26, begin=0.3, cycle=15, cls="ink sans", glyph_seed=6)
    s.append(f'<g font-weight="800">{title_svg}</g>')
...
def soundtrack_svg(c):
    ...
    s.append(f'<g font-weight="800">{title_svg}</g>')
    s.append(f'<text x="40" y="63" font-size="11" class="dim">Mikky Ekko \u2014 click to play'
              f'<tspan class="cursor{uid}">_</tspan></text>')
...
# ---------------------------------------------------------------- CAPACITY (the pill, the letter rain, the meter)
def clarity_rain(c, uid, x, y, w, h, cols=None, begin=0):
    """falling gold letters/equations — the Limitless-pill-scene homage, kept lean on purpose"""
    cols = cols or max(8, int(w / 34))
    out = [f'<g clip-path="url(#clip{uid})" opacity="0.55">']
    r = random.Random(stable_hash(uid) + 11)
    for i in range(cols):
        cx_ = x + i * (w / cols) + (w / cols) / 2
        dur = round(r.uniform(3.2, 7.0), 2)
        delay = round(r.uniform(0, 5), 2)
        chars = [r.choice(CLARITY_CHARS) for _ in range(6)]
        col = [f'<g><animateTransform attributeName="transform" type="translate" '
               f'values="0,{-h}; 0,{h}" dur="{dur}s" begin="{delay}s" repeatCount="indefinite"/>']
        for j, ch in enumerate(chars):
            op = round(max(1 - j * 0.16, 0.05), 2)
            fill = c["accent3"] if j == 0 else c["accent"]
            col.append(f'<text x="{cx_:.1f}" y="{y + j*16}" font-size="12" fill="{fill}" opacity="{op}">{esc(ch)}</text>')
        col.append('</g>')
        out.append("".join(col))
    out.append('</g>')
    return "\n".join(out)
...
def capacity_svg(c):
    ...
    uid = "y"
    cx = WIDTH / 2
    s = [svg_open(h), defs_and_style(c, uid), panel(c, h, uid, sweep_dur=9, sweep_begin=-3)]

    s.append(clarity_rain(c, uid, 4, -20, WIDTH - 8, h + 20, cols=16))
    s.append(f'<rect x="4" y="4" width="{WIDTH-8}" height="{h-8}" rx="20" fill="{c["panel"]}" opacity="0.62"/>')

    title_svg, _ = decode_text(c, uid, 40, 44, "FULL CAPACITY", 19, begin=0.2, cycle=8, cls="ink sans", glyph_seed=31)
    s.append(f'<g font-weight="800">{title_svg}</g>')
    s.append(f'<text x="40" y="63" font-size="11" class="dim">every pattern, all at once \u2014 the margins keep talking'
              f'<tspan class="cursor{uid}">_</tspan></text>')

    # the pill — two-tone capsule, pulsing
    px, py = 92, 145
    s.append(f'<g>'
              f'<animateTransform attributeName="transform" type="translate" values="0,0;0,-3;0,0" dur="1.8s" repeatCount="indefinite"/>'
              f'<circle cx="{px}" cy="{py}" r="26" fill="url(#core{uid})" opacity="0.5">'
              f'<animate attributeName="opacity" values="0.35;0.6;0.35" dur="1.8s" repeatCount="indefinite"/></circle>'
              f'<g transform="rotate(-32 {px} {py})">'
              f'<path d="M {px-18} {py} a18 18 0 0 1 18 -18 h6 v36 h-6 a18 18 0 0 1 -18 -18 Z" fill="{c["accent2"]}"/>'
              f'<path d="M {px+18} {py} a18 18 0 0 1 -18 18 h-6 v-36 h6 a18 18 0 0 1 18 18 Z" fill="{c["accent3"]}"/>'
              f'<rect x="{px-1}" y="{py-18}" width="2" height="36" fill="{c["ink2"]}" opacity="0.5"/>'
              f'</g></g>')

    # cognitive-capacity meter — a filling bar with checkpoint numbers lighting up as it passes
    bx, by, bw_, bh_ = 200, 175, WIDTH - 200 - 40, 10
    s.append(f'<text x="{bx}" y="{by-10}" font-size="10.5" class="dim" letter-spacing="0.5">COGNITIVE CAPACITY</text>')
    s.append(f'<rect x="{bx}" y="{by}" width="{bw_}" height="{bh_}" rx="5" fill="{c["off"]}" stroke="{c["line"]}" stroke-width="1"/>')
    s.append(f'<rect x="{bx}" y="{by}" width="0" height="{bh_}" rx="5" fill="{c["accent"]}">'
              f'<animate attributeName="width" values="0;{bw_};{bw_};0;0" keyTimes="0;0.55;0.62;0.7;1" dur="7s" repeatCount="indefinite"/></rect>')
    checkpoints = [("10%", 0.10), ("34%", 0.34), ("61%", 0.61), ("89%", 0.89), ("100%", 1.0)]
    for label, frac in checkpoints:
        lx = bx + bw_ * frac
        light_at = 0.55 * frac
        kt = f"0;{max(light_at-0.02,0):.2f};{light_at:.2f};0.55;0.62;0.7;1"
        s.append(f'<circle cx="{lx:.1f}" cy="{by+bh_/2:.1f}" r="2.4" fill="{c["accent3"]}" opacity="0">'
                  f'<animate attributeName="opacity" values="0;0;1;1;0;0;0" keyTimes="{kt}" dur="7s" repeatCount="indefinite"/></circle>')
        s.append(f'<text x="{lx:.1f}" y="{by+24}" text-anchor="middle" font-size="9" class="dim2">{esc(label)}</text>')

    # a few clarity-flash words — short, unquoted, movie-adjacent in spirit only
    for word, wx, wy, delay in [("FOCUS", WIDTH-160, 100, 2), ("PATTERNS", WIDTH-190, 210, 7), ("CLARITY", 60, 240, 12)]:
        s.append(whisper(c, uid, wx, wy, word, cycle=16, begin=delay, cls="accent3", anchor="start"))
