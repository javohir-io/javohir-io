...
def header_svg(c):
    ...
    s.append(laughing_skull(c, uid, WIDTH-92, 86, scale=1.0))
...
def soundtrack_svg(c):
    ...
    title_svg, _ = decode_text(c, uid, 40, 44, "SOUNDTRACK", 19, begin=0.2, cycle=8, cls="ink sans", glyph_seed=21)
    s.append(f'<g font-weight="800">{title_svg}</g>')
    s.append(f'<text x="40" y="63" font-size="11" class="dim">coding playlist \u2014 GitHub won\u2019t play audio here, so click through and actually hit play'
              f'<tspan class="cursor{uid}">_</tspan></text>')
    ...
    for i, hh in enumerate(heights):
        ...
        s.append(f'<rect x="{bx}" y="{158-hh:.1f}" width="6" height="{hh:.1f}" rx="2" fill="{c["accent"]}">'
                  ...
                  f'</rect>')
    s.append(f'<text x="{eqx}" y="178" font-size="10.5" class="dim">frontend focus \u00b7 lofi \u00b7 synthwave \u00b7 late-night debugging music</text>')
